...
def main():
    ...
    for i, (batch, extra) in enumerate(data):
        ...
        for index in range(images.shape[0]):
            ...
            base_dir_2, filename = os.path.split(extra["filepath"][index])
            base_dir_2 = './translated/'
            filename, ext = filename.split(".")
            filepath_2 = os.path.join(base_dir_2, f"{filename}_translated_{target_y_list[index]}.{ext}")
            image = Image.fromarray(images[index])
            image.save(filepath_2)
